...
import { ArrowLeft, Plus, X, Calendar, Tag, BookOpen, CheckCircle, Share2, Link as LinkIcon } from 'lucide-react';
...
export default function Stories() {
  ...
  const shareStory = (story, platform) => {
    ...
    const text = `"${story.title}" — One Good Word, One Good Deed`;
          ...
          >
            ...
            >
              ...
              <div className="mt-8 pt-6 border-t border-white/[0.08]">
                <p className="font-barlow-condensed text-cream/30 text-xs uppercase tracking-widest mb-3 flex items-center gap-2">
                  <Share2 className="w-3.5 h-3.5" /> Share This Story
                </p>
                <div className="flex items-center gap-2 flex-wrap">
                  <button
                    onClick={() => shareStory(selected, 'facebook')}
                    className="flex items-center gap-2 px-4 py-2 bg-[#1877F2]/10 hover:bg-[#1877F2]/20 border border-[#1877F2]/30 text-[#1877F2] font-barlow-condensed text-xs uppercase tracking-wider rounded-sm transition-all"
                  >
                    <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                    Facebook
                  </button>
                  <button
                    onClick={() => shareStory(selected, 'twitter')}
                    className="flex items-center gap-2 px-4 py-2 bg-black/20 hover:bg-black/40 border border-white/10 text-cream/70 hover:text-cream font-barlow-condensed text-xs uppercase tracking-wider rounded-sm transition-all"
                  >
                    <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                    X / Twitter
                  </button>
                  <button
                    onClick={() => shareStory(selected, 'copy')}
                    className="flex items-center gap-2 px-4 py-2 bg-gold/10 hover:bg-gold/20 border border-gold/20 text-gold font-barlow-condensed text-xs uppercase tracking-wider rounded-sm transition-all"
                  >
