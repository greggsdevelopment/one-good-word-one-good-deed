    ...
    <>
      <AnimatePresence>
        ...
        <motion.div
          ...
          className="fixed z-50 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-[#0f0f12] border border-cream/10 rounded-sm shadow-2xl"
        >
          <button
            onClick={onClose}
            className="absolute top-4 right-4 z-10 text-cream/30 hover:text-cream transition-colors bg-ink/80 rounded-sm p-1"
          >
            <X className="w-5 h-5" />
          </button>
          ...
          <div className="flex flex-col sm:flex-row">
            {/* Image */}
            <div className="sm:w-1/2 h-64 sm:h-auto bg-white/[0.03] flex items-center justify-center shrink-0">
              {product.image ? (
                <img src={product.image} alt={product.name} className="w-full h-full object-cover" />
              ) : (
                <p className="font-anton text-cream/20 text-2xl tracking-widest">PHOTO COMING</p>
              )}
            </div>
            ...
            <div className="flex-1 p-6 flex flex-col gap-4">
              <div>
                <p className="font-barlow-condensed text-gold/60 text-xs tracking-[0.3em] uppercase mb-1">{product.category}</p>
              ...
              {product.sizes?.length > 0 && (
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <p className="font-barlow-condensed text-cream/40 text-xs tracking-widest uppercase">Size</p>
                    <button
